...
class Confession(BaseCog):
    ...
    async def confess(self, ctx, *, confession):
        ...
        """
        confession = discord.Embed(title="Anonymous Confession", description=confession, colour=0x8ff1ff,timestamp=datetime.now())
        ...
        for guild in all_guilds:
            if guild.get_member(ctx.message.author.id):
                ...
                if room is not None and room is not "":
    ...
    async def send_confession(self, ctx, confession_guild, confession):
        ...
        await ctx.author.send("Your confession has been sent to: "+ str(confession_guild))
