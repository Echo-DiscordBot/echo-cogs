...
class Utilities(commands.Cog):
    ...
    async def iplookup(self, ctx, arg):
        ...
        embed.add_field(name="IP", value="\n\u200b".join(str(values['query'])), inline=False)       
        embed.add_field(name="Country", value="\n\u200b".join(str(values['country'])), inline=True)
        embed.add_field(name="Region", value="\n\u200b".join(str(values['regionName'])), inline=True)
        embed.add_field(name="City", value="\n\u200b".join(str(values['city'])), inline=True)
        embed.add_field(name="Latitude", value="\n\u200b".join(str(values['lat'])), inline=True)
        embed.add_field(name="Longitude", value="\n\u200b".join(str(values['lon'])), inline=True)
        embed.add_field(name="Timezone", value="\n\u200b".join(str(values['timezone'])), inline=True)
        embed.add_field(name="ISP", value="\n\u200b".join(str(values['isp'])), inline=True)
