...
#                 data = json.loads(await resp.read(), object_hook=DictObject)
            ...
            embd.add_field(name="IP Address:", value=ipaddr, inline=True)
            embd.add_field(name="ISP Name/Organization:", value=organization, inline=True)
            embd.add_field(name="City:", value=city, inline=True)
            embd.add_field(name="Regional Area:", value=region, inline=True)
            embd.add_field(name="Region Code:", value=region_code, inline=True)
            embd.add_field(name="Country:", value=country, inline=True)
            embd.add_field(name="Country Name:", value=country_name, inline=True)
            embd.add_field(name="Country Code (ISO):", value=country_code_iso3, inline=True)
            embd.add_field(name="Language Spoken:", value=languages, inline=True)
            embd.add_field(name="Continent Code:", value=continent_code, inline=True)
            embd.add_field(name="Is country a member of European Union (EU)?", value=in_eu, inline=True)
            embd.add_field(name="Postal Code:", value=postal, inline=True)
            embd.add_field(name="Latitude Coordinate:", value=latitude, inline=True)
            embd.add_field(name="Longitude Coordinate:", value=longitude, inline=True)
            embd.add_field(name="Timezone:", value=country_timezone, inline=True)
            embd.add_field(name="UTC Offset:", value=utc_offset, inline=True)
            embd.add_field(name="Country Dial Code:", value=dial_code, inline=True)
            embd.add_field(name="Currency:", value=currency, inline=True)
            embd.add_field(name="Autonomous System Number:", value=asn, inline=True)
