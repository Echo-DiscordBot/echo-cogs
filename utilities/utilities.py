...
class Utilities(commands.Cog):
    ...
    async def iplookup(self, ctx, arg):
        ...
        embed.add_field(name="IP", value="\u200b".join(values['query']), inline=False)       
        embed.add_field(name="Country", value="\u200b".join(values['country']), inline=True)
        embed.add_field(name="Region", value="\u200b".join(values['regionName']), inline=True)
        embed.add_field(name="City", value="\u200b".join(values['city']), inline=True)
        embed.add_field(name="Latitude", value="\u200b".join(values['lat']), inline=True)
        embed.add_field(name="Longitude", value="\u200b".join(values['lon']), inline=True)
        embed.add_field(name="Timezone", value="\u200b".join(values['timezone']), inline=True)
        embed.add_field(name="ISP", value="\u200b".join(values['isp']), inline=True)
